...
SpaceRiderGame::SpaceRiderGame()
{

	
}

void SpaceRiderGame::render()

{
	...
	drawMesh(m_sky_mesh, m_list_materials);
		...
		// Render Asteroids on the jupiter
		for (int i = 0; i < 4; i++)
		{
			renderHelper(m_asteroid_mat, m_asteroid_mesh, getPositionofRevolvings(m_jupiter_pos, 4000.0 + 200 * i, 4000.0f + 200 * i, i), Vector3D(0, 0.9 * m_time, 0), Vector3D(100, 100, 100));
		}
		renderHelper(m_asteroid_mat, m_asteroid_mesh, getPositionofRevolvings(m_earth_pos, 4000.0 + 200 , 4000.0f + 200 , 1), Vector3D(0, 0.9 * m_time, 0), Vector3D(100, 100, 100));
...
void SpaceRiderGame::updateModel(Vector3D position, Vector3D rotation, Vector3D scale,const std::vector<MaterialPtr>& list_materials)
{
	constant cc;
...
void SpaceRiderGame::updateCamera()
{
...
void SpaceRiderGame::updateThirdPersonCamera()
{
...
void SpaceRiderGame::updateSkyBox()
{
	constant cc;
...
void SpaceRiderGame::updateLight()
{
	//strikewithplanets();
	Matrix4x4 temp;
	temp.setIdentity();
...
void SpaceRiderGame::updateSpaceship()
{
...
void SpaceRiderGame::drawMesh(const MeshPtr& mesh, const std::vector<MaterialPtr>& list_materials)
{
...
void SpaceRiderGame::onDestroy()
{
	Window::onDestroy();
	m_swap_chain->setFullScreen(false, 1, 1);
}

void SpaceRiderGame::onFocus()
{
	InputSystem::get()->addListener(this);
}

void SpaceRiderGame::onKillFocus()
{
	InputSystem::get()->removeListener(this);
}

void SpaceRiderGame::onSize()
{
	RECT rc = this->getClientWindowRect();
...
void SpaceRiderGame::onKeyDown(int key)
{
...
void SpaceRiderGame::onKeyUp(int key)
{
	m_forward = 0.0f;
	m_rightward = 0.0f;
...
void SpaceRiderGame::onMouseMove(const Point& mouse_pos)
{
	if (!m_play_state) return;
...
void SpaceRiderGame::onLeftMouseDown(const Point& mouse_pos)
{
	if (!m_play_state) {
		m_play_state = true;
		InputSystem::get()->showCursor(!m_play_state);
...
void SpaceRiderGame::onLeftMouseUp(const Point& mouse_pos)
{
	
}

void SpaceRiderGame::onRightMouseDown(const Point& mouse_pos)
{
	
}

void SpaceRiderGame::onRightMouseUp(const Point& mouse_pos)
{
	
}

void SpaceRiderGame::checkCollision()
{
...
void SpaceRiderGame::strikewithplanets()
{
...
Vector3D SpaceRiderGame::getPositionofRevolvings(Vector3D center, float longradii, float shortradii)
{
...
Vector3D SpaceRiderGame::getPositionofRevolvings(Vector3D center, float longradii, float shortradii, float angle)
 {
	if (m_time < 0.01)
	{
...
void SpaceRiderGame::renderHelper(MaterialPtr mat,MeshPtr mesh, Vector3D pos, Vector3D rot, Vector3D scale)
{
	m_list_materials.clear();
...
void SpaceRiderGame::normalizeCamera()
{
